...
def test_multimodal_indexer_uses_single_batched_loop(
    ...
    assert indexer.sparse_model is sparse
    ...
    samples = [
        MultimodalSample(
            ...
            document_id=str(i),
            text=f"body{i}",
            modalities=[MultimodalRawInput(type="image", value=f"/f{i}.png")],
            metadata=DocumentMetadata(),
        )
        for i in range(2)
    ]
    with (
        patch.object(sparse, "embed_documents", wraps=sparse.embed_documents) as spy,
        patch.object(indexer.client, "insert", wraps=indexer.client.insert) as insert,
    ):
        assert indexer.index_documents(samples, "col") == 2

    # One batched pass, both models loaded: Qwen sees the multimodal text,
    # SPLADE only the plain text.
    assert insert.call_count == 1
    assert dense.embed_documents.call_args[0][0] == [
        MultimodalEmbeddings._multimodal_to_text(s) for s in samples
    ]
    assert spy.call_args[0][0] == ["body0", "body1"]
    rows = insert.call_args.kwargs["data"]
    assert [json.loads(row["image_paths"]) for row in rows] == [
        ["/f0.png"],
        ["/f1.png"],
    ]


def test_chunker_keeps_modality_of_single_attachment():
    sample = MultimodalSample(
        text="see <attachment> here",
        modalities=[MultimodalRawInput(type="image", value="/a.png")],
        metadata=DocumentMetadata(),
    )
    chunk = Chunk(
        text=sample.text, start_index=0, end_index=len(sample.text), token_count=4
    )
    assert MultimodalChunker._chunk_modalities(sample, [chunk]) == [sample.modalities]
